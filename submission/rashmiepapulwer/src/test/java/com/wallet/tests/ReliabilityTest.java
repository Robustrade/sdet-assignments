...
class ReliabilityTest extends WalletTransferTestBase {
    ...
    void concurrentDuplicateRequests_exactlyOneTransferRow() throws InterruptedException {
        ...
        assertThat(completed)
                .as("Concurrent requests did not complete within %d seconds",
                        AWAIT_TIMEOUT_SECS)
                .isTrue();

        assertThat(responses)
                .as("Expected one HTTP response from every worker thread. " +
                        "A missing response usually indicates an uncaught exception " +
                        "or thread failure before the request completed.")
                .hasSize(THREAD_COUNT);

// Exactly one transfer row should exist for the shared idempotency key.
    ...
    void concurrentTransfersToSameDestination_allCreditsApplied() throws InterruptedException {
        ...
        for (int i = 0; i < senderCount; i++) {
            String senderId = walletFixture.createFundedWallet("USD", each.multiply(BigDecimal.valueOf(2)));
