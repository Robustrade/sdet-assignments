...
If row-locking is missing, `balance_of(db, wallets.a)` returns `-20` and the test fails hard. This is the "lost-update / overdraw" concurrency test.
...
4. **Concurrency tests are stochastic.** They will nearly always catch a lost-update bug on the first run, but a single passing run isn't proof of correctness under all interleavings. To shake harder, the Step-2 deliverable will pin [`pytest-repeat`](https://pypi.org/project/pytest-repeat/) in `requirements.txt` and document `pytest --count=20 -m reliability` in the README (the `--count` flag is provided by that plugin, not by pytest itself). CI will run the reliability suite once by default and can be re-triggered with `--count=20` on demand.
