...
/**
 * Fluent assertions for wallet balance state verification.
 *
 * Captures before/after snapshots so each assertion reads as:
 *   "source wallet balance decreased by the transfer amount"
 *
 * Usage:
 *   WalletBalance sourceBefore = WalletAssertions.snapshot(dbClient, sourceId);
 *   // ... initiate transfer ...
 *   WalletAssertions.assertThat(dbClient, sourceId)
 *       .balanceDecreasedBy(new BigDecimal("100.00"))
 *       .comparedTo(sourceBefore);
 */
