        ...
        cursor.execute("""
            ...
            VALUES
              ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
               ...
               10.00, 'USD')
        """)
        fail("Self-transfer INSERT succeeded — chk_transfers_different_wallets not enforced")
        conn.rollback()
    except psycopg2.errors.CheckViolation:
        ok("Self-transfer correctly rejected by chk_transfers_different_wallets")
        conn.rollback()
    except Exception as e:
        fail(f"Unexpected error during self-transfer check: {e}")
        conn.rollback()
...
def main():
    schema_path = os.path.realpath(SCHEMA_FILE)
    print(f"Schema file : {schema_path}")
    print(f"Target DB   : {DB_CONFIG['user']}@{DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['dbname']}")

    if not os.path.isfile(schema_path):
        print(f"\nERROR: Schema file not found at {schema_path}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        with conn.cursor() as cur:
            print("\n[0] Applying schema.sql")
            cur.execute(schema_sql)
            conn.commit()
            ok("schema.sql applied without errors")
        ...
        with conn.cursor() as cur:
            ...
            check_balance_non_negative_enforced(conn, cur)
            conn.commit()

            check_self_transfer_rejected(conn, cur)
            conn.commit()
    ...
    print("\n" + "=" * 60)
    if failures:
        print(f"SCHEMA VALIDATION FAILED — {len(failures)} issue(s):")
        for f in failures:
            print(f"  - {f}")
        sys.exit(1)
    else:
        print(f"SCHEMA VALIDATION PASSED — all checks OK")
        sys.exit(0)
